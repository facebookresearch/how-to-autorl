...
from run_agent import WandbCallback, BufferedVecMonitor, ToBaselinesVecEnv, ResNetCNN, BraxToSBWrapper

log = logging.getLogger(__name__)


@hydra.main(config_path="configs", config_name="dehb")
def train_sb3(cfg: DictConfig):
    ...
    if cfg.env_name in ENV_NAMES:
        ...
        venv = gym.wrappers.TransformReward(venv, lambda reward: np.clip(reward, -10, 10))
        env = BufferedVecMonitor(env=venv, filename=f"{cfg.log_dir}/logs.txt", buffer_size=10000)
        ...
        model_kwargs["policy_kwargs"] = {"net_arch": [{"pi": [64, 64], "vf": [64, 64]}], "activation_fn": torch.nn.SiLU}
        ...
        env = BufferedVecMonitor(env=env, filename=f"{cfg.log_dir}/logs.txt", buffer_size=10000)
        env.reset()
    else:
        env = gym.make(cfg.env_name)
        env = Monitor(env, cfg.log_dir)
    ...
    if cfg.load:
        ...
        model = agent_class(cfg.algorithm.policy_model, env, **model_kwargs, verbose=2)
        ...
        try:
            model.learn(
                ...
                callback=WandbCallback(
                    custom_vec_monitor=cfg.env_name in ENV_NAMES or cfg.env_name in ["ant", "halfcheetah", "humanoid"]
                ),
            )
        except:
            if cfg.save:
